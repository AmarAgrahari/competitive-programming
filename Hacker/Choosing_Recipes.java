	...
	public static void main(String[] args) {
		  Scanner in = new Scanner(System.in);
	        ...
	        for(int a0 = 0; a0 < q; a0++){
	            ...
	            int minimum_recipe[]=new int[r];int sum=0;
	            for(int recipe_i=0; recipe_i < r; recipe_i++){
	                for(int recipe_j=0; recipe_j < p; recipe_j++){
	                    recipe[recipe_i][recipe_j] = in.nextInt();
	                   sum=sum+(recipe[recipe_i][recipe_j]*cost[recipe_j]);
	                }
	                minimum_recipe[recipe_i]=sum;
	                sum=0;
	            }
	            
	            Arrays.sort(minimum_recipe);
	          
	            int t=0;
	            for(int i=0;i<n;i++)
	            {
	            	t=t+minimum_recipe[i];
	            }
	            
	            int min=Integer.MAX_VALUE;
	            for(int i=0;i<m;i++)
	            {
	            	int diff=t-cost[pantry[i]];
	            	if(diff<=min)
	            	{
	            		min=diff;
	            	}
